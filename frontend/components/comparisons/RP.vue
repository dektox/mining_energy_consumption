<template>
    <v-flex my-4 pa-3>
        <v-layout align-center justify-center>
            <h2 class="display-1">
                Total World Renewables Production
            </h2>
        </v-layout>
        <v-layout v-bind="binding" align-center justify-center my-3>
            <v-flex xs3 ma-3 class="text-xs-center">
                <v-card elevation="5">
                    <v-layout pa-4 column align-center justify-center>
                        <v-flex>
                            <span class="title"><b>Biofuel &amp; waste<br><br></b></span>
                        </v-flex>
                        <v-layout column align-center justify-center my-3>
                            <img src="~static/images/comparisons/biofuel.png" object-fit="contain" width="30%">
                            <span><b>577 TWh</b></span>
                        </v-layout>
                        <v-flex>
                            <p>Could power the entire Bitcoin network</p>
                        </v-flex>
                        <v-layout align-center justify-center my-3>
                            <div class="image-cropper-sm">
                                <img class="image-pic" src="data:image/png;base64,">
                            </div>
                        </v-layout>
                        <v-flex>
                            <span style="font-size: 32px"><b>{{ 577 / numbers2[0] | round }} times</b></span>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex xs3 ma-3 class="text-xs-center">
                <v-card elevation="5">
                    <v-layout pa-4 column align-center justify-center>
                        <v-flex>
                            <span class="title"><b>Solar, wind &amp;<br> geothermal</b></span>
                        </v-flex>
                        <v-layout column align-center justify-center my-3>
                            <img src="~static/images/comparisons/alternative.png" object-fit="contain" width="30%">
                            <span><b>1 405 TWh</b></span>
                        </v-layout>
                        <v-flex>
                            <p>Could power the entire Bitcoin network</p>
                        </v-flex>
                        <v-layout align-center justify-center my-3>
                            <div class="image-cropper-sm">
                                <img class="image-pic" src="data:image/png;base64,">
                            </div>
                        </v-layout>
                        <v-flex>
                            <span style="font-size: 32px"><b>{{ 1405 / numbers2[0] | round }} times</b></span>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex xs3 ma-3 class="text-xs-center">
                <v-card elevation="5">
                    <v-layout pa-4 column align-center justify-center>
                        <v-flex>
                            <span class="title"><b>Hydro<br><br></b></span>
                        </v-flex>
                        <v-layout column align-center justify-center my-3>
                            <img src="~static/images/comparisons/hydro.png" object-fit="contain" width="30%">
                            <span><b>4 164 TWh</b></span>
                        </v-layout>
                        <v-flex>
                            <p>Could power the entire Bitcoin network</p>
                        </v-flex>
                        <v-layout align-center justify-center my-3>
                            <div class="image-cropper-sm">
                                <img class="image-pic" src="data:image/png;base64,">
                            </div>
                        </v-layout>
                        <v-flex>
                            <span style="font-size: 32px"><b>{{ 4164 / numbers2[0] | round }} times</b></span>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
        </v-layout>
        <v-layout my-3>
          <span>
              <b>Source:</b><br/>
              International Energy Agency <a src="">report</a>, 2016 est.
          </span>
        </v-layout>
    </v-flex>
</template>

<script>
    import {percentage, decimals, round} from '~/assets/js/filters.js'

    export default {
        name: 'ComparisonsCards',
        filters: {
            percentage,
            decimals,
            round
        },
        data() {
            return {
            }
        },
        computed: {
            numbers() {
                return this.$store.state.numbers
            },
            numbers2() {
                const data  = [...this.$store.getters.GET_DATA].pop() || {}
                return [data.guess_consumption || 0, data.min_consumption || 0, data.max_consumption || 0]
            },
            binding() {
                const binding = {}
                if (this.$vuetify.breakpoint.xsOnly) binding.column = true
                return binding
            }
        }
    }
</script>
